        ...
        for row in df.itertuples(name="ADPUserList"):
            ...
            if employee_status == "Terminated":
                ...
                if return_dict:
                    ...
                    print(
                        f"User found in the MSGraph dictionaries. Now deleting {employee_full_name}!\n"
                    )
                    # Trying some threading here...
                    t = threading.Thread(target=delete_user, args=employee_email)
                    # Need to append this to a list of threads, or is this enough for this specific case?
                    # delete_user(employee_email)
                else:
                    print(
                        f"{employee_full_name} termed on {employee_separation_date}, not found in MS info. Should be deleted already.\n"
                    )
                    logging.info(
                        f"{employee_full_name} termed on {employee_separation_date}, not found in MS info. They should have already been deleted.\n"
                    )
